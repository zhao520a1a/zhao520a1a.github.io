...
- **比较推荐使用 Sonic 库**，因不论从性能和功能总体而言，Sonic 的表现的确很亮眼；此外，通过了解 Sonic 的内部实现原理，也为对于 cpu 密集性的操作的优化提供一种“野路子，即：通过编写高性能的 C 代码并经过优化编译后供 Golang 直接调用。但其实并不，因为实际上 Go 源码中的一些 cpu 密集型操作底层就是编译成了汇编后使用的，如：crypto 和 math。
